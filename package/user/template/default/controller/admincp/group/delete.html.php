...
<div><input type="hidden" name="val[delete_id]" value="{$aGroup.user_group_id}" /></div>
	{_p var='are_you_sure_you_want_to_delete_the_user_group_title' title=$aGroup.title|clean}
	...
	<input type="submit" value="{_p var='delete_this_user_group'}" class="button btn-primary" />.

	<br />
	
	<input type="button" value="{_p var='no_thanks_get_me_out_of_here'}" class="button" onclick="window.location.href = '{url link='admincp.user.group'}';" />
