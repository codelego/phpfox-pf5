...
class User_Service_Api extends \Core\Api\ApiServiceBase
{
    ...
    {
        $aUser = Phpfox::get('user')->get($params['id'], true, false);
        ...
        if (Phpfox::get('user.block')->isBlocked(null, $iUserId))
        {
    ...
    {
        $aUser = Phpfox::get('user')->get($params['id'], true);
        if (empty($aUser) || empty($aUser['user_id']))
        {
            return $this->error(_p('The {{ item }} cannot be found.', ['item' => _p('user__l')]));
        }
    ...
    public function post()
    {
        ...
        if ($oValid->isValid($aVals))
        {
            ...
            {
                \Phpfox::get('user.auth')->setUserId($iId);
